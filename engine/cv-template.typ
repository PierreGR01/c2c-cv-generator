...
#set text(font: c2c-font, size: 8.5pt, fill: c2c-ink, lang: "fr")
...
#set list(
  ...
  body-indent: 5pt,
  spacing: 0.45em,
  marker: text(fill: c2c-orange, size: 10pt)[›],
)
...
#let section(title) = {
  block(breakable: false, above: 15pt, below: 9pt)[
    #stack(
      dir: ttb,
      spacing: 5pt,
      text(fill: c2c-orange, weight: "black", size: 10pt)[#title],
...
#grid(
  ...
  [
    #text(weight: "black", size: 22pt, fill: c2c-title)[#id.at("prenom") #id.at("nom")]
    #v(-2pt)
    #text(weight: "semibold", size: 12pt, fill: c2c-orange)[#id.at("poste")]#if id.at("departement", default: "") not in (none, "") [
      #h(6pt)#text(size: 9pt, fill: c2c-grey, weight: "regular")[· #id.at("departement")]
    ]
  ],
)
...
#let info-row(label, value) = {
  if value != none and value != "" {
    block(below: 5pt)[
      #text(weight: "light", size: 8pt)[#label : ]#text(weight: "bold", size: 8pt)[#value]
    ]
  } else { none }
}
...
#let competences = cv.at("competences", default: ())
  ...
  let comp-items-size = cv.at("comp_items_font", default: 7.5) * 1pt
  ...
  let comp-cell(c) = grid(
    ...
    align: (top + left, top + left),
    column-gutter: 5pt,
    text(size: 7pt, fill: c2c-orange, weight: "bold")[#c.at("categorie")],
...
#let certs = cv.at("certifications", default: ())
  ...
  for c in certs [
    - #text(weight: "semibold", size: 8.5pt)[#c.at("intitule")]
      #text(fill: c2c-grey, size: 8.5pt)[— #c.at("organisme")#if c.at("annee", default: none) != none [, #str(c.at("annee"))]]
  ]
}
...
#let projets = cv.at("projets", default: ())
  ...
  for (i, p) in projets.enumerate() {
    ...
    let techs   = p.at("competences", default: p.at("technologies", default: ()))
    ...
    block(breakable: false, above: if i == 0 { 6pt } else { 15pt })[
      ...
      #grid(
        ...
        let col-droite = {
          if techs.len() > 0 {
            text(size: 7pt, fill: c2c-grey, weight: "bold")[Compétences]
            v(1pt)
            if techs.len() > 5 {
              ...
              grid(
                ...
                align: (top + left, top + left),
                {
                  ...
                  for t in half1 [- #text(size: 7.5pt, fill: c2c-ink)[#t]]
                },
                {
                  set list(marker: text(fill: c2c-grey, size: 8pt)[·], indent: 0pt, body-indent: 4pt, spacing: 0.32em)
                  for t in half2 [- #text(size: 7.5pt, fill: c2c-ink)[#t]]
                },
              )
            } else {
              set list(
              ...
              for t in techs [- #text(size: 7.5pt, fill: c2c-ink)[#t]]
            }
          }
        }
        v(3pt)
  ...
  for (i, e) in parcours.enumerate() {
    ...
    block(spacing: 0pt)[
      #grid(
        ...
        column-gutter: 10pt,
        align: (top + left, top + left),
        text(fill: c2c-grey, size: 8pt, weight: "semibold")[#e.at("periode")],
        // Poste à gauche, organisation justifiée à droite
        grid(
          columns: (1fr, auto),
          column-gutter: 8pt,
          align: (left + top, right + top),
          text(size: 8.5pt, weight: "semibold")[#e.at("poste")],
          if e.at("employeur", default: "") != "" [
            #text(size: 8pt, fill: c2c-grey)[#e.at("employeur")]
          ] else [],
        ),
      )
    ]
